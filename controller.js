...
const onlineFullControllerMode = params.get("onlineControl") === "1" || params.get("onlineSensor") === "1";
const onlineSensorMode = onlineFullControllerMode;
...
const initialRoom = (onlineFullControllerMode ? onlineSensorRoom : (params.get("room") || "")).toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 6);
...
function reportSensorStatus(active = sensorStreaming, force = false) {
  ...
  const eventName = onlineFullControllerMode ? "online:controller-status" : "controller:sensor-status";
  const payload = onlineFullControllerMode ? { active: normalized, capabilities } : { roomCode: joinedRoom, active: normalized, capabilities };
...
function attachOnlineFullController() {
  if(!onlineFullControllerMode||!socket||!onlineSensorRoom||!onlineSensorPlayerId||!onlineSensorToken)return;
  document.body.classList.add("online-full-controller");
  joinScreen?.classList.add("hidden");
  controlScreen?.classList.remove("hidden");
  boardControllerArea?.classList.add("hidden");
  sensorModePanel?.classList.add("hidden");
  connectionBadge.textContent="Conectando controle…";
  sensorModeRequested=true;

  socket.timeout(MULTIPLAYER_TIMEOUT_MS).emit(
    "online:controller-join",
    {roomCode:onlineSensorRoom,playerId:onlineSensorPlayerId,resumeToken:onlineSensorToken},
    (error,response)=>{
      if(error||!response?.ok){
        connectionBadge.textContent="Falha no pareamento";
        turnKicker.textContent="Controle Online";
        turnTitle.textContent="Não foi possível parear";
        turnDescription.textContent=response?.message||"Abra novamente o QR Code mostrado na sua tela Online.";
        return;
      }
      myPlayerId=response.playerId;
      joinedRoom=response.roomCode;
      playerLabel.textContent=response.name||"Jogador Online";
      playerColor.style.background=response.color||"#45d9ff";
      connectionBadge.textContent="Controle pareado";
      sensorModeRequested=true;
      window.dispatchEvent(new CustomEvent("steam-online-controller-joined",{detail:{roomCode:joinedRoom,playerId:myPlayerId,state:response.state||null,onlineV2State:response.onlineV2State||null,controlMethod:response.controlMethod||"phone-motion"}}));
      reportSensorStatus(sensorStreaming,true);
    }
  );
}
...
if (socket) {
  ...
  socket.on("connect", () => {
    if (onlineFullControllerMode) { attachOnlineFullController(); return; }
...
if(onlineFullControllerMode&&socket?.connected)setTimeout(attachOnlineFullController,0);

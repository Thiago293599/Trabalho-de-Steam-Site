(() => {
  ...
  function renderReplicaMinigame(){
    ...
    if(phase==="minigame-intro"){minigameIntro?.classList.remove("hidden");droneGame?.classList.add("hidden");motionGame?.classList.add("hidden");minigameResult?.classList.add("hidden");document.getElementById("finalDanceSongSelection")?.classList.add("hidden");if(minigameTitle)minigameTitle.textContent=mg?.title||"Minigame";if(minigameDescription)minigameDescription.textContent="Aguardando o host iniciar. Você joga nesta própria tela.";startMinigame?.classList.add("hidden");return}
    if(phase==="just-dance-select"){minigameIntro?.classList.add("hidden");droneGame?.classList.add("hidden");motionGame?.classList.add("hidden");minigameResult?.classList.add("hidden");startMinigame?.classList.add("hidden");window.STEAMPartyBoard?.renderDanceSongSelectionReplica?.(mg||{});return}
    document.getElementById("finalDanceSongSelection")?.classList.add("hidden");

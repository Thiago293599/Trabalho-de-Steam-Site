(() => {
  ...
  function renderMinigame(){
    ...
    round.textContent=`Rodada ${partyState.round} / ${partyState.totalRounds}`;
    score.textContent=`${Math.round(me()?.score||0)} PTS`;
    if(miniTopic)miniTopic.textContent=`Minigame • ${p.topic||"Matemática"}`;
    if(miniTitle)miniTitle.textContent=partyState.minigame?.title||"Desafio STEAM";
    question.textContent=p.question||"Escolha a resposta correta.";
    ...
    (p.answers||[]).forEach(value=>{
      ...
      b.type="button";
      b.className="final-party-phone-answer";
      const displayed=Number.isInteger(Number(value))?String(Number(value)):String(Math.round(Number(value)*10)/10);
      b.textContent=p.unit==="°"?`${displayed}°`:`${displayed} ${p.unit||""}`.trim();
      b.disabled=answeredToken===activeProblemToken;
      b.onclick=()=>{
        if(answeredToken===activeProblemToken)return;
        answeredToken=activeProblemToken;
        answers.querySelectorAll("button").forEach(x=>x.disabled=true);
        b.classList.add("is-selected");
        emit("minigame-answer",{answer:Number(value),problemToken:activeProblemToken,clientTime:Date.now()});
      };
